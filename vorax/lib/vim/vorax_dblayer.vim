...
function s:db.Connect(cstr) dict
  ...
  if g:vorax_store_passwords && self.connected
...
function s:GetPassword(profile)
  let pwd_repo = substitute(fnamemodify(expand('$HOME'), ':p:8') . '/_vorax_passwords', '\\', '/', 'g')
  let private_key_file = substitute(fnamemodify(expand('$HOME'), ':p:8') . '/vorax_key', '\\', '/', 'g')
  let public_key_file = substitute(fnamemodify(expand('$HOME'), ':p:8') . '/vorax_key.pub', '\\', '/', 'g')
  if s:ProfileInRepo(a:profile, pwd_repo) && !s:ProfileSkip(a:profile, pwd_repo)
    let info = {'error' : "", 'pwd' : ""}
    " look for the private key
    if !filereadable(private_key_file)
      ...
      for i in range(3)
        ...
        let info = s:GetPwdFromRepo(a:profile, pwd_repo, private_key_file, public_key_file)
    ...
    if !filereadable(private_key_file)
      ...
      if user_response == "" || user_response ==? 'y'
        ...
        while 1
          ...
          if s:master_password != retype_master_password
            ...
            let error = s:GenerateRsaKeys(private_key_file, public_key_file)
            let error = s:WriteToPwdRepo(a:profile, pwd, pwd_repo, private_key_file, public_key_file)
            if error != ''
              redraw
      ...
      if !s:ProfileInRepo(a:profile, pwd_repo) && !s:ProfileSkip(a:profile, pwd_repo)
        ...
        if user_response ==? 'e'
          ...
          let pwd = ""
          let error = s:WriteToPwdRepo(a:profile, pwd, pwd_repo, "", "")
          if error != ''
            redraw
            call s:tk_utils.EchoErr(g:vorax_messages["invalid_password"] . "\n" . error)
          endif
