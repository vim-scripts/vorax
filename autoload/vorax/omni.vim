...
function! s:WordItems(prefix) abort"{{{
  ...
  let local_items = s:LocalItems(a:prefix)
...
function! s:LocalItems(prefix) abort"{{{
  " where are we?
	let result = {'resultset' : [[]]}
	let crr_pos = s:context['absolute_pos']
	
	" tell me more about the code structure
	let text_code = vorax#utils#BufferContent(1, line('.')) . a:prefix
	let items = vorax#ruby#LocalItems(text_code, crr_pos-1, '')
	for item in items
